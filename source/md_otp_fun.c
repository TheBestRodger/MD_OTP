...
#include "md_otp.h"
/*
    Структура - описать где используется
*/
...
/*
 Объявленный метод - описать
*/
static void request_send(request *req);

/*
 Вот что она делает?
 @brief read_secret_file
 @param secret_file
 @param secret
...
{
    com_err("libOTP", 0, "Loading read_secret_file");
    ...
    *secret = k5memdup0(&buf[i], j - i, &retval);
    com_err("libOTP", retval, "Loadedread_secret_file");
cleanup:
    free(filename);
    return retval;
}
...
/* 
    ...
    Инициализираует начальными значениями поля структуры
    Можно сказать реалацирует память здесь для стуктуры token_type
*/
...
{
    com_err("libOTP", 0, "Loading token_type_decode");
    ...
    indicators = NULL;
    com_err("libOTP", retval, "Loaded token_type_decode");
cleanup:
    free(name_copy);
    free(server);
    free(secret);
...
token_types_decode(profile_t profile, token_type **out)
{
    ...
    {
        printf("subsection_names name [%d] : %s\n", iii, names[i]);
        ++iii;
    }
    ...
    *out = types;
    types = NULL;
    com_err("libOTP", retval, "Loaded token_types_decode");
cleanup:
...
{
    com_err("libOTP", 0, "token_decode %d", 55055);
    ...
    char * us_name = (char*) val;
    
    com_err("libOTP", 0, "%s", us_name);
    if (val != NULL && k5_json_get_tid(val) == K5_JSON_TID_STRING) {
...
otp_state_new(krb5_context ctx, otp_state **out)
{
    ...
    if (retval != 0)
        return retval;
    com_err("libOTP",0,"hostname = %s", hostname);
    ...
    if (retval != 0)
        goto error;
    com_err("libOTP",0,"Loading md_krad_attrset_new");
    retval = md_krad_attrset_new(ctx, &self->attrs);
    if (retval != 0)
        goto error;
    com_err("libOTP",0,"Loading md_krad_attrset_add");
    hndata = make_data(hostname, strlen(hostname));
    retval = md_krad_attrset_add(self->attrs,
                               krad_attr_name2num("NAS-Identifier"), &hndata);
    if (retval != 0)
        goto error;
    com_err("libOTP",0,"Loading md_krad_attrset_add_number");
    retval = md_krad_attrset_add_number(self->attrs,
...
// /* единоличная функция вызывается в main.c*/
// void
// otp_state_verify(otp_state *state, verto_ctx *ctx, krb5_const_principal princ,
//                  const char *config, const krb5_pa_otp_req *req,
//                  otp_cb cb, void *data)
// {
// //     krb5_error_code retval; // показатель ошибки, главное чтоб не 0
// //     request *rqst = NULL; // запрос
// //     char *name;
